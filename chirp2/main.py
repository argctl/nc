...
# Noise cancellation via low-frequency phase inversion
def flip_low_freq_phase(audio_chunk):
    y = audio_chunk.flatten()
    Y = rfft(y)
    freqs = rfftfreq(len(y), 1/fs)
    # Invert only frequencies below the cutoff
    Y_flipped = np.where(freqs < low_freq_cutoff, -Y, Y)
...
print("📡 Gitarg Pulse-Chirp with Low-Freq Noise Cancellation Activated. Ctrl+C to stop.")
...
def callback(indata, outdata, frames, time, status):
    ...
    # Flip low-frequency phase for noise cancellation
    flipped = flip_low_freq_phase(indata)
