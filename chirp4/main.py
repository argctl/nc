...
# Remove high frequencies and those that persist for too long
def cancel_persistent_and_high_freqs(audio_chunk):
    ...
    # Zero out persistent bins and high frequencies
    for i, f in enumerate(freqs):
        if f > high_freq_cutoff:
            Y[i] = 0
        elif freq_count.get(i, 0) >= len(freq_history):
            Y[i] = 0
...
print("📡 Gitarg Pulse-Chirp with Persistent & High Frequency Cancellation Activated. Ctrl+C to stop.")
...
def callback(indata, outdata, frames, time, status):
    ...
    cleaned = cancel_persistent_and_high_freqs(indata)
