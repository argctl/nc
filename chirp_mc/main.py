...
def callback(indata, outdata, frames, time_info, status):
    ...
    if prev_magnitudes is not None:
        ...
        if len(matching_bins) > 0 and (now - last_chirp_time) > chirp_debounce:
            base_freq = np.mean(rfftfreq(len(input_audio), 1/fs)[matching_bins])
            chirp_signal = generate_chirp(f0=base_freq + 200, f1=base_freq - 200)
            last_chirp_time = now
