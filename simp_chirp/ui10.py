...
def callback(indata, outdata, frames, time_info, status):
    ...
    now = time.time()
    hash_recent = hash_digest in hash_timestamps and (now - hash_timestamps[hash_digest]) <= chirp_debounce

    chirp_signal = np.zeros(frames, dtype=np.float32)

    if constant_enabled and (now - last_constant_chirp) > chirp_debounce:
        chirp_signal = generate_multi_chirp()
    ...
    hash_timestamps[hash_digest] = now
