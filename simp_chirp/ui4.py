...
def launch_ui():
    ...
    def update_plot():
        while True:
            ...
            for i, trail_wave in enumerate(waveform_trail):
                alpha = 0.1 + 0.1 * i
            ...
            ax.plot(waveform_data[1] * chirp_mag, color='red', label="Chirp")
            ax.legend()
            canvas.draw()
            canvas.flush_events()
            time.sleep(0.05)
    ...
    trail_slider = tk.Scale(root, from_=1, to=20, resolution=1,
